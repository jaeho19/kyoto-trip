...
## 특징
- 📴 **오프라인 우선**: 서비스워커가 앱 셸·이미지·폰트 캐시 → 일정·장소·정보 탭은 데이터로밍 없이 사용(지도 탭은 구글 지도라 네트워크 필요).
- ⚖️ **저작권 안전**: 장소 사진은 Wikimedia Commons의 **CC/PD** 이미지만 번들, 저작자·라이선스 표기(정보 탭). 실제 사진이 없는 곳은 분위기 **대표 이미지**로 표기.
- 🧩 **번들 우선**: 폰트 self-host, 아이콘 인라인 SVG(CDN JS 의존 0). 지도는 키 없는 구글 지도 임베드(+차량 길찾기 딥링크).
- 🗺️ 4탭: 일정(Day 카드·D-day) · 장소(분류·일자 필터) · **지도(구글 지도 일자별 차량 동선)** · 정보.
