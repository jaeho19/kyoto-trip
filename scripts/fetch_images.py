...
REJECT_TITLE = re.compile(r"(mcdonald|food court|toilet|map of|diagram|logo|signboard)", re.I)

# Representative (대표) images for places with NO exact free image (CC0/free, marked honestly).
# KUAS tries its real former name first (京都学園大学/Kyoto Gakuen University) → may hit actual campus.
REPRESENTATIVE = {
    "kuas": ["京都先端科学大学", "京都学園大学 太秦", "Kyoto Gakuen University", "Kyoto University of Advanced Science"],
    "hotel_kyoto": ["ryokan room tatami Kyoto", "Japanese hotel room tatami", "Kyoto Karasuma street"],
    "hotel_kobe": ["Kobe Sannomiya night view", "Kobe cityscape", "Kobe harbor night"],
    "gochisomura": ["Japanese set meal teishoku", "washoku set meal", "Japanese lunch tray"],
    "yakinikuking": ["yakiniku grilled beef", "yakiniku restaurant grill", "grilled wagyu beef"],
    "arashiyama_resto": ["obanzai Kyoto cuisine", "Japanese buffet food", "washoku kaiseki dishes"],
    "kagonoya": ["Japanese teishoku tray meal", "washoku set meal tray", "tempura set meal"],
    "takadanobaba": ["oden Japanese hotpot", "oden dish", "Japanese oden"],
}
...
def find_image_multi(terms):
    """Try several search terms; return first free hit."""
    for t in terms:
        hit = find_image(t)
        if hit:
            return hit, t
...
def main():
    images, credits, representative = {}, [], []

    # 1) exact landmark images
    for key, term in TARGETS.items():
        ...
        try:
            ...
            continue
        images[key] = fname
        credits.append({"key": key, "title": TARGETS[key], "author": author,
                        "license": lic, "licenseUrl": licurl, "sourceUrl": descurl})
        print(f"  -> {fname} ({sz//1024} KB) [{lic} / {author}]")
        time.sleep(0.5)

    # 2) representative (대표) images for places with no exact free image
    for key, terms in REPRESENTATIVE.items():
        if key in images:
            continue  # already has an exact image
        print(f"[{key}] (representative) {terms[0]} ...")
        hit, used = find_image_multi(terms)
        if not hit:
            print("  -> none free; design-card fallback")
            continue
        thumb, descurl, lic, author, licurl = hit
        try:
            ...
            continue
        images[key] = fname
        representative.append(key)
        credits.append({"key": key, "title": f"{used} (대표 이미지)", "author": author,
                        "license": lic, "licenseUrl": licurl, "sourceUrl": descurl,
                        "representative": True})
        print(f"  -> {fname} ({sz//1024} KB) [{lic} / {author}] (rep: '{used}')")
        time.sleep(0.5)
    ...
    js = (
        ...
        "export const REPRESENTATIVE = " + json.dumps(representative, ensure_ascii=False) + ";\n"
        "export const CREDITS = " + json.dumps(credits, ensure_ascii=False, indent=2) + ";\n"
    )
    (ROOT / "js" / "assets.js").write_text(js, encoding="utf-8")
    print(f"\nWrote js/assets.js: {len(images)} images ({len(representative)} representative), {len(credits)} credits")
