...
## v3 보강 (2026-06-09) — 로고 + 전 장소 사진 + MapLibre 3D 지도

- **서울시립대 로고**: 헤더에 공식 UOS 로고 적용(라이트=컬러/다크=화이트 자동 전환, RISE 프로젝트 자산 재사용).
- **전 장소 사진**: 사진 없던 8곳(호텔2·KUAS·식당5)에 Wikimedia Commons **무료(CC/CC0)** 이미지 추가.
  실제 그 장소가 아닌 분위기 이미지는 **"대표 이미지"** 배지로 정직하게 표기. KUAS는 실제 명칭(京都先端科学大学)
  검색으로 실 캠퍼스 사진 매칭. → 16곳 전부 사진(디자인 카드 0).
- **지도 MapLibre GL로 교체**: CARTO 벡터 스타일(라이트/다크), **3D 건물 extrusion·pitch·회전**,
  **일자별 색상 동선(Day1 빨강/Day2 초록/Day3 파랑)**, **일자 필터 칩**, 마커 일자 배지,
  **사진 썸네일 팝업**, 타임라인 클릭 → 해당 핀으로 flyTo 연동.
- **오프라인 트레이드오프**: 지도 벡터 타일/스타일은 CARTO 온라인 의존(방문 영역만 SW 캐시) — 사용자 선택(MapLibre).
- **재검증**: 로컬+라이브 Playwright PASS — 사진 16/디자인 0, MapLibre 마커 16, 크레딧 16, 콘솔에러 0.

## 잔여 / 사용자 작업

1. 고치소우무라 다카이시점 정확 지번은 미확정(시 중심 좌표 사용) — 예약 확정 시 갱신 가능.
2. 식당·호텔 일부는 분위기 **대표 이미지**(CC) — 실제 지점 공식 사진으로 교체 원하면 교체 가능.
